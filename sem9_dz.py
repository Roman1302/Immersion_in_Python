...
def create_csv_file():
    with open('random_numbers.csv', 'w', encoding='UTF-8') as file:
        writer = csv.writer(file, quoting=csv.QUOTE_NONNUMERIC)
        for row in range(random.randint(STR_MIN, STR_MAX)):
...
def json_result(func: Callable):
    ...
    def wrapper(*args):
        roots = func(*args)
        solve_dict = {'a': args[0], 'b': args[1], 'c': args[2], 'result': roots}
