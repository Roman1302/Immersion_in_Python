...
class Student:
    ...
    def __repr__(self):
        result = f'''Ученик "{self.name} {self.second_name} {self.surname}",
...
if __name__ == '__main__':
    mik = Student("Иван", "Петрович", "Сидоров", Path('lessons.csv'))
    # print(mik)
    mik.new_estimate("русский язык", 5)
    mik.new_estimate("литература", 5)
    mik.new_estimate("физика", 2)
    mik.new_estimate("физика", 4)
    mik.new_estimate("геометрия", 5)
    mik.new_estimate("русский язык", 2)
    mik.new_estimate("химия", 3)
    mik.new_estimate("физика", 5)
    mik.new_estimate("физика", 2)
    mik.new_estimate("алгебра", 4)
    mik.new_estimate("геометрия", 82, "тест")
    mik.new_estimate("алгебра", 99, "тест")
    mik.new_estimate("русский язык", 29, "тест")
    mik.new_estimate("русский язык", 100, "тест")
    mik.new_estimate("история", 18, "тест")
    mik.new_estimate("география", 74, "тест")
    mik.new_estimate("русский язык", 39, "тест")
    mik.new_estimate("литература", 89, "тест")
    print(mik)
